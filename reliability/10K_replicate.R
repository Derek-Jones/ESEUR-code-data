...
lines(sort(f_times, na.last=TRUE), 1:length(f_times), type="b",
...
plot(1, log="x", type="n",
        xlim=c(1, max_test), ylim=c(1, ncol(T1)),
        xlab="Input cases", ylab="Failures experienced")
dummy=sapply(2:nrow(T1), function(X) plot_fail_time(T1[X, ], X))

max_test=max(T3, na.rm=TRUE)
plot(1, log="x", type="n",
        xlim=c(1, max_test), ylim=c(1, ncol(T3)),
        xlab="Input cases", ylab="Failures experienced")
dummy=sapply(2:nrow(T3), function(X) plot_fail_time(T3[X, ], X))
