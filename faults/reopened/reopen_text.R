...
build_tdm=function(fault_text, remod)
{
desc_text=Corpus(VectorSource(fault_text))
desc_text=tm_map(desc_text, removePunctuation)
desc_text=tm_map(desc_text, tolower)
# Stemming has loads of dependencies I have not bothered with
# desc_text=tm_map(desc_text, stemDocument)
desc_text=tm_map(desc_text, removeWords, stopwords("english"))

# Matrix containing count of each word in each 'document'
tdm <<- TermDocumentMatrix(desc_text)
...
reopened_data=read.csv(paste0(ESEUR_dir, "faults/reopened/reopened_fault.data.csv.xz"), as.is=TRUE)

build_tdm(reopened_data$description, reopened_data$remod)
...
build_tdm(reopened_data$comments, reopened_data$remod)
