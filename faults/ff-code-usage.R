...
# Model used to predict internet users during year
user_mod=glm(Developed ~ year, data=i_users)

usage_list$i_users=predict(user_mod, newdata=data.frame(year=usage_list$week))


# Plot total usage for version 1.0 source
t_usage=total_code_usage("v1.0")
xbound=c(min(t_usage$week), max(t_usage$week))
ybound=c(0, max(t_usage$code_usage))

plot(t_usage$week, t_usage$code_usage, type="n",
	yaxs="i",
	xlim=xbound, ylim=ybound,
	xlab="Date", ylab="End-user code usage\n")
...
{
q=subset(t_usage, rep_id == ver_str)
lines(q$week, q$LOC*q$i_users*q$market_share, col=ver_col)
}
...
points(t_usage$week, t_usage$code_usage, col=pal_col[1])
