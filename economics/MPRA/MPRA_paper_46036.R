...
plot(2:120, c(max(y[2:120]), rep(0, length(y[2:120])-1)), col="White", type="l", 
...
a_projectkalman_original_params=function(xx)
{
#Parameters
#Provided parameters
a=xx[1]
...
plot(1:(timeperiods-1), c(0, rep(max(predicted_pirate_acquisitions), timeperiods-2)), col="White", type="l", 
	xlab="Time", ylab="Units")
lines(1:(timeperiods-1), actual_sales, col=pal_col[1])
lines(1:(timeperiods-1), predicted_sales, col=pal_col[2])
lines(1:(timeperiods-1), predicted_pirate_acquisitions, col=pal_col[3])

legend(x="bottomright", legend=c("Actual", "Predicted sales", "Predicted pirated"), bty="n", fill=pal_col, cex=1.2)
...
# #Draw the data
# ##############################################
# #Predicted sales with actual sales
