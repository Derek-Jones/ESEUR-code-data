...
x[i]<-(a+(alpha/(1+max(Y[i-1],1)^epsilon))*(b1*X[i-1]+b2*Y[i-1])/NObs[i])*(NObs[i]-X[i-1]-Y[i-1])
y[i]<-(1-alpha/(1+max(Y[i-1],1)^epsilon))*((b1*X[i-1]+b2*Y[i-1])/NObs[i])*(NObs[i]-X[i-1]-Y[i-1])
...
X<-X[52]+cumsum(data$WordProcessors) #So that X[1] is now January 1987
Y[1]<-Y[52]
...
X_pred[1]<-X1+((a+(alpha/(1+max(X2,1)^epsilon))*(b1*X1+b2*X2)/N[j*steps+i])*(N[j*steps+i]-X1-X2))*dt
X_pred[2]<-X2+((1-(alpha/(1+max(X2,1)^epsilon)))*((b1*X1+b2*X2)/N[j*steps+i])*(N[j*steps+i]-X1-X2))*dt

F11<-((alpha/(1+max(X2,1)^epsilon))*b1/N[j*steps+i])*(N[j*steps+i]-X1-X2)-(a+(alpha/(1+max(X2,1)^epsilon))*(b1*X1+b2*X2)/N[j*steps+i])
F12<-(-epsilon*max(X2,1)^(epsilon-1)*(alpha/(1+max(X2,1)^epsilon)^2)*((b1*X1+b2*X2)/N[j*steps+i])+(alpha/(1+max(X2,1)^epsilon))*(b2/N[j*steps+i]))*(N[j*steps+i]-X1-X2)-(a+(alpha/(1+max(X2,1)^epsilon))*(b1*X1+b2*X2)/N[j*steps+i])
F21<-((1-alpha/(1+max(X2,1)^epsilon))*b1/N[j*steps+i])*(N[j*steps+i]-X1-X2)-(1-(alpha/(1+max(X2,1)^epsilon)))*((b1*X1+b2*X2)/N[j*steps+i])
F22<-(epsilon*max(X2,1)^(epsilon-1)*(alpha/(1+max(X2,1)^epsilon)^2)*((b1*X1+b2*X2)/N[j*steps+i])+(1-alpha/(1+max(X2,1)^epsilon))*(b2/N[j*steps+i]))*(N[j*steps+i]-X1-X2)-(1-(alpha/(1+max(X2,1)^epsilon)))*((b1*X1+b2*X2)/N[j*steps+i])
...
start_params<-c(a,b1,b2,alpha,q11)

trial_params<-c(((start_params[1]/0.02)/(1-(start_params[1]/0.02)))^0.5,(start_params[2]/(1-start_params[2]))^0.5,(start_params[3]/(1-start_params[3]))^0.5,(start_params[4]/(1-start_params[4]))^0.5,0.5)

estvals<-optim(trial_params,projectkalman,hessian=TRUE,control=list(trace=0,maxit=2000,fnscale=-1))

pr<-estvals$par

estpars<-c(0.02*pr[1]^2/(1+pr[1]^2),pr[2]^2/(1+pr[2]^2),pr[3]^2/(1+pr[3]^2),pr[4]^2/(1+pr[4]^2),pr[5]^2/(1+pr[5]^2))
