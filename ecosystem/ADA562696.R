...
aircraft=read.csv(paste0(ESEUR_dir, "ecosystem/ADA562696.csv.xz"), as.is=TRUE)
...
dis_vec(cur, decade+2, pal_col[1])

out=subset(ac, Status == "Out")
dis_vec(out, decade-2, pal_col[2])

fut=subset(ac, Status == "Future")
dis_vec(fut, decade-2, pal_col[3])
...
legend(x="topleft", legend=c("Flying", "Withdrawn", "Planned"), bty="n", fill=pal_col, cex=1.2)
